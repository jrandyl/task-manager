...
import (
	...
	"github.com/jrandyl/task-manager/backend/internal/config"
)
...
func loadConfigFromURL() (*pgxpool.Config, error) {
	...
	if !ok {
		return nil, fmt.Errorf("Must set the DATABASE_URL environment variable")
	}
...
func dbURL() (string, error) {
	...
	if err != nil {
		dbURL, ok := os.LookupEnv("DATABASE_URL")
		if !ok {
			return "", fmt.Errorf("Must set the DATABASE_URL environment variable")
		}
